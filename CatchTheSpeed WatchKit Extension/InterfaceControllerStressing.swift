...
class InterfaceControllerStressing: WKInterfaceController {
...
//        var params = ["game_type":InterfaceControllerGlobal.ModeGame.stressing.rawValue]
//        var whereClause : NSString?
//        do{
//            var whereClause = (NSString(data: try NSJSONSerialization.dataWithJSONObject(params, options: NSJSONReadingOptions.MutableContainerscan), encoding: NSUTF8StringEncoding))?.stringByAddingPercentEncodingWithAllowedCharacters(NSCharacterSet.URLQueryAllowedCharacterSet())
//        }catch {
//            whereClause = nil;
//        }
...
//        var urlReq = Utility.prepareRestRequest("https://api.parse.com/1/classes/Points?order=-score&limit=10&include=user&where=\(whereClause!)")
         ...
         var urlReq = Utility.prepareRestRequest("https://api.parse.com/1/classes/Points?order=-score&limit=10&include=user&where=\(InterfaceControllerGlobal.ModeGame.stressing.rawValue)")
        
        urlReq.addValue("application/json", forHTTPHeaderField: "Content-Type")
        urlReq.addValue("application/json", forHTTPHeaderField: "Accept")
        
        let queue:NSOperationQueue = NSOperationQueue()
        NSURLConnection.sendAsynchronousRequest(urlReq, queue: queue, completionHandler:{
